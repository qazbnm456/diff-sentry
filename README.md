...
| Deterministic detectors | `indicators.py` | Pure-Python, in-loop-safe (no subprocess): `${IFS}`, `curl \| bash`, base64 de-obfuscation, CODEOWNERS/workflow tamper, `permissions: write-all` escalation, `pull_request_target`-checks-out-PR-head (pwn-request), CI-skip bypass, secret/`/proc/<pid>/mem` exfiltration, known OAST/exfil callback domains and IPFS/permaweb gateways, Node execution primitives (detached spawn, `node -e`, fetch-to-disk dropper), `_0x…` obfuscated identifiers, diff-presentation evasion (whitespace shove, bidi/zero-width), prompt-injection. |
