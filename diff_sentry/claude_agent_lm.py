"""Run diff-sentry's planner + analyst on a Claude Pro/Max SUBSCRIPTION via the Claude Agent SDK.

VENDORED from rlm-kit `examples/claude_agent_lm.py` (github.com/qazbnm456/rlm-kit), pinned to commit
48374a1 — the vendored CODE body (below the docstring) matches that commit verbatim. Three docstring/demo
adaptations: the two demo-only imports (pydantic, rlm_kit) and the upstream demo tail (the `Summarize`
RLMTask + `main()` runner) are dropped, and this module docstring adds the diff-sentry header above the
preserved upstream docstring — whose Setup step 3 is adjusted to `uv sync --extra subscription` since the
runnable demo it referenced is gone. rlm-kit ships this adapter only under `examples/`, which is NOT in
its installed wheel, so diff-sentry vendors a copy — per the base/wrap split, the LM provider lives in
the consumer. Keep edits here minimal so the diff against upstream stays legible; re-sync on an rlm-kit
bump. Provenance + how-to-update in `VENDOR.md`.

Imported LAZILY (only inside `detect.setup()`'s subscription branch, via `_maybe_subscription_lm`),
never at a module top and never from `diff_sentry/__init__.py`, so `import diff_sentry` stays dspy-free
and a proxy-only install never needs the `[subscription]` extra. Enabled by giving DS_ROOT_LM and/or
DS_SUB_LM a `claude-agent-sdk/<model>` value (see the subscription block in `.env.example`); the
CLASSIFIER always stays on its own OpenAI-compatible endpoint (DS_CLASSIFIER_LM), never the subscription.
...
Setup:
  ...
  3. `uv sync --extra subscription` (or `uv pip install "claude-agent-sdk>=0.1.60"`), then
     `brew install deno` for the default pyodide sandbox.
