...
diff-sentry deliberately vendors **almost nothing** — it is a downstream *consumer* of rlm-kit (a
commit-pinned git dep), and the deterministic detection logic (indicators, assemble, emit) is its own
...
- **Pinned ref: `48374a1`** (rlm-kit `main` HEAD). The vendored **code body (below the docstring)
  ...
  gone. diff-sentry now consumes rlm-kit as a **commit-pinned git source**, and `uv.lock` currently pins
  that same `48374a1` — the vendored copy and the installed kit coincide today, no divergence to note.
  The adapter is a standalone `dspy.BaseLM` depending only on the long-stable
  `configure(main_lm=, sub_lm=)` seam, so the pin need not track the lockfile — but re-check this one
  file whenever `examples/claude_agent_lm.py` changes upstream, and whenever the lockfile pin moves,
  note any divergence here (like the siblings do).
